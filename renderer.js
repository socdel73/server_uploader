...
const btnRemoteUp = document.getElementById("btnRemoteUp");
...
function updateUploadRow(
  uploadId,
  { transferredBytes, totalBytes, percent, speed, eta, status },
) {
  if (!uploadsList) return;
  const row = uploadsList.querySelector(
    `.upload-row[data-upload-id="${uploadId}"]`,
  );
  if (!row) return;
  ...
  const totalText = totalBytes > 0 ? bytesToHuman(totalBytes) : "—";
  const xferText = Number.isFinite(transferredBytes)
    ? bytesToHuman(transferredBytes)
    : "0 B";
...
function getActiveProfile() {
  ...
  const profile =
    activeProfile ||
    (configCache?.profiles ? configCache.profiles[name] : null);
  if (!name || !profile)
...
function initProfiles(cfg) {
  ...
  const profileDefault =
    (cfg.profiles[defaultName] && cfg.profiles[defaultName].remoteDirDefault) ||
    "";
  const globalDefault = cfg.remoteDirDefault || "";

  // Default shown in UI: per-profile default, falling back to global default.
  remoteDirDefaultLabel.textContent =
    profileDefault || globalDefault || "(not set)";

  // Current value: in-memory override for this profile, otherwise the defaults.
  remoteDirByProfile[defaultName] =
    remoteDirByProfile[defaultName] || profileDefault || globalDefault || "";
  remoteDirInput.value = remoteDirByProfile[defaultName];

  // Remote roots per profile (if defined)
  const roots = cfg.profiles[defaultName].remoteRoots ||
    cfg.remoteRoots || ["/srv/storage-media", "/srv/storage-2md"];
  ...
  return new Promise((resolve, reject) => {
    ...
    proc.on("close", (code) => {
      ...
      if (code === 2 || out.startsWith("NOT_A_DIR:")) {
        ...
        return reject(
          new Error(`Remote path exists but is not a directory: ${p}`),
        );
      }
      return reject(
        new Error(`ssh ensureRemoteDir exit ${code}\n${stderr.trim()}`),
      );
    });
  });
}
...
function listRemoteDirs(profile, rootPath, currentPath = rootPath) {
  return new Promise((resolve, reject) => {
    const allowed = profile.remoteRoots ||
      configCache?.remoteRoots || ["/srv/storage-media", "/srv/storage-2md"];

    if (!allowed.includes(rootPath)) {
      reject(new Error("Root path no permès"));
      return;
    }
    ...
    if (
      ...
      reject(new Error("Current path fora del root permès"));
      return;
    }
    ...
    const safePath = normalizedCurrent.replace(/'/g, `'\\''`);
    const findCmd = `find '${safePath}' -mindepth 1 -maxdepth 1 -type d -not -path '*/.*' -print | sort`;
...
btnUpload.addEventListener("click", async () => {
  ...
  try {
    ...
    if (!remoteDir) {
      throw new Error(
        "Remote dir not set. Enter a server path like /srv/storage-media/uploads_test (per-profile default is shown below).",
      );
    }
    ...
    const settled = await runPool(
      files.map((localFile, idx) => ({ localFile, idx })),
      MAX_PARALLEL,
      async ({ localFile, idx }) => {
        ...
        try {
          const result = await uploadFile(
            ...
            remoteDirResolved,
            uploadId,
            ({ transferredBytes, percent, speed, eta }) => {
              updateUploadRow(uploadId, {
                transferredBytes,
                totalBytes,
                percent,
          ...
          log(
            `ERROR (${idx + 1}/${files.length}): ${String(err.message || err)}`,
          );
          throw err;
        }
      },
...
profileSelect.addEventListener("change", () => {
  ...
  const profileDefault =
    (configCache.profiles[name] &&
      configCache.profiles[name].remoteDirDefault) ||
    "";
  ...
  remoteDirByProfile[name] =
    remoteDirByProfile[name] || profileDefault || globalDefault || "";
  remoteDirInput.value = remoteDirByProfile[name];

  const roots = configCache.profiles[name].remoteRoots ||
    configCache.remoteRoots || ["/srv/storage-media", "/srv/storage-2md"];
...
if (btnFetchRemoteDirs && remoteDirList) {
  btnFetchRemoteDirs.addEventListener("click", async () => {
    try {
      ...
      currentRemotePath = root;

      remoteDirList.innerHTML = '<option value="">(carregant...)</option>';

      const dirs = await listRemoteDirs(profile, root, currentRemotePath);
      ...
      if (!dirs.length) {
        const opt = document.createElement("option");
        opt.value = "";
        opt.textContent = "(sense subcarpetes)";
        remoteDirList.appendChild(opt);
      }

      remoteDirHint.textContent = `Directori actual: ${currentRemotePath}`;
    } catch (err) {
...
if (btnUseRemoteDir && remoteDirList) {
  btnUseRemoteDir.addEventListener("click", async () => {
...
if (btnRemoteUp) {
  btnRemoteUp.addEventListener("click", async () => {
    try {
      const root = remoteRootSelect.value;
      const { profile } = getActiveProfile();

      // si ja estem al root, no pugem més
      if (!currentRemotePath || currentRemotePath === root) {
        remoteDirHint.textContent = "Ja estàs al directori base";
        return;
      }

      // calcular pare
      let parent = currentRemotePath.replace(/\/$/, "");
      parent = parent.substring(0, parent.lastIndexOf("/"));

      // evitar sortir del root
      if (!parent.startsWith(root)) {
        parent = root;
      }

      currentRemotePath = parent;
      remoteDirInput.value = parent;
...
if (btnComposeRemote && newSubfolderInput) {
  btnComposeRemote.addEventListener("click", async () => {
    try {
      const root = remoteRootSelect?.value || "";
      const { profile } = getActiveProfile();

      const baseFromList =
        currentRemotePath || remoteDirList.value || root || "";
      const sub = newSubfolderInput.value.trim();

      if (!baseFromList) {
        remoteDirHint.textContent =
          "Selecciona una base o carpeta llistada primer.";
        return;
      }

      if (!sub) {
        remoteDirHint.textContent = "Escriu el nom de la subcarpeta nova.";
        return;
      }

      const fullPath = joinRemotePath(baseFromList, sub);

      await ipcRenderer.invoke("createRemoteDir", profile, fullPath);
      currentRemotePath = fullPath;

      remoteDirInput.value = fullPath;
      ...
      remoteDirHint.textContent = `Directori actual: ${currentRemotePath}`;
      log(`Carpeta remota creada: ${fullPath}`);
    ...
    if (chkDeleteOnCancel?.checked) {
      for (const [uploadId, info] of entries) {
        ...
        try {
          ...
          log(
            `ERROR esborrant remot (${uploadId}): ${String(err.message || err)}`,
          );
        }
      }
    }
